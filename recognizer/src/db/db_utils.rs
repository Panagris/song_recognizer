...
use std::env;
use crate::db::models::{Fingerprint, NewFingerprint, Song, NewSong};
use crate::recognizer::declarations::{DATABASE_INSERT_ERROR, DATABASE_QUERY_ERROR, NO_SONG_MATCH_ERROR};
...
pub fn store_fingerprints(fingerprint_map: HashMap<u32, KeyAudioPoint>) -> Result<(), u8> {
    ...
    for (hash , point) in fingerprint_map {
        ...
        let new_fingerprint = NewFingerprint { hash: hash as i32 , anchor_time_ms, song_id };

        match diesel::insert_into(fingerprints::table)
            .values(&new_fingerprint).execute(connection) {
            Ok(_) => {},
            Err(_) => {
...
pub fn get_key_audio_points(hashes: Vec<i32>) -> Result<HashMap<u32, Vec<KeyAudioPoint>>,
    u8> {
    use crate::db::schema::fingerprints;
    ...
    for hash in hashes {
        ...
        let matching_points: Vec<Fingerprint> = match fingerprints::table
            .filter(fingerprints::hash.eq(hash)).load::<Fingerprint>(connection)
        {
            Ok(v) => v,
            Err(_) => return Err(DATABASE_QUERY_ERROR),
        };

        let points_from_db = matching_points.into_iter().map(|fingerprint: Fingerprint| {
            KeyAudioPoint { anchor_time_ms : fingerprint.anchor_time_ms , song_id : fingerprint.song_id }
...
pub fn store_song(title: &String, artist: &String, spotify_uri: String) -> Result<u32, u8> {
    ...
    let new_post = NewSong { title: title.to_owned(), artist: artist.to_owned(), spotify_uri, 
        song_key };

    let song: Song = match diesel::insert_into(songs::table).values(&new_post)
        .returning(Song::as_returning()).get_result(connection)
    {
...
pub fn get_song_by_id(song_id: u32) -> Result<Song, u8> {
    ...
    let matching_songs: Vec<Song> = match songs::table
        .filter(songs::id.eq(song_id as i32)).load::<Song>(connection)
    {
        Ok(songs) => songs,
        Err(_) => return Err(DATABASE_QUERY_ERROR)
    };
